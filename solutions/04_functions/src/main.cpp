...
static volatile uint32_t const *const GPIOD_idr =
...
inline void moder_set_as_output(volatile uint32_t *const reg, uint32_t bit) {
  assert(bit < 16u); // 16 bits per port (0-15)
  auto moder = *reg;
...
inline void moder_set_as_input(volatile uint32_t *const reg, uint32_t bit) {
  assert(bit < 16u); // 16 bits per port (0-15)
  auto moder = *reg;
  moder &= ~(0b11u << (bit * 2)); // set as input (clear mode bits)
  *reg = moder;
}

inline void odr_set(volatile uint32_t *const reg, uint32_t pattern) {
  auto value = *reg;
  value |= (pattern & 0xFFFFu); // 16 bits per port (0-15)
  *reg = value;
}
